...
export default async function FantasySection({ hideSummaries = false }: FantasySectionProps) {
  ...
  return (
    <section className="home-section-surface relative py-16 px-4 lg:px-8 2xl:px-12 3xl:px-16">
      ...
      <div className="relative mx-auto max-w-[84rem] 2xl:max-w-[94rem] 3xl:max-w-[106rem] z-10">
        <div className="mb-4 2xl:mb-6 3xl:mb-8"><div className="flex flex-wrap items-center gap-8 mb-3"><h2 className="text-xl sm:text-xl 2xl:text-2xl 3xl:text-3xl font-bold text-neutral-700">Fantasy Football</h2></div></div>
            ...
            <div key={article._id || index}>
              {article.slug?.current ? (
                <Link
                  ...
                  className="group flex gap-3 rounded-2xl bg-neutral-50 p-3 transition-all duration-300 hover:bg-neutral-50"
                >
                  ...
                  <div className="min-w-0 flex-1 pt-0.5">
                    <span className="mb-1 inline-flex rounded-full bg-neutral-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-neutral-600">
                      Fantasy
                    </span>
                    <h3 className="line-clamp-2 text-[15px] font-semibold leading-snug text-neutral-900 group-hover:text-neutral-700">
                      {article.title}
                    </h3>
                    {article.summary && !hideSummaries && (
                      <p className="mt-1 line-clamp-2 text-[11px] leading-snug text-neutral-500">{article.summary}</p>
                    )}
                    {article.author?.name && (
                ...
                <div className="rounded-2xl bg-neutral-50 p-3 text-sm text-gray-500">No content</div>
              )}
            </div>
          ))}
        </div>
              ...
              {article && article.slug?.current ? (
                <Link href={toFantasyUrl(article)} className="group">
                  <div className="space-y-3">
                    <div className="relative h-[200px] 2xl:h-[220px] 3xl:h-[250px] rounded-xl overflow-hidden bg-neutral-100 hover:bg-neutral-100 transition-all duration-500 hover:scale-[1.01] shadow-sm hover:shadow-md">
                      {article.coverImage?.asset ? (<Image src={urlFor(article.coverImage).width(800).height(600).fit('crop').url()} alt={article.title} fill className="object-cover object-left-top transition-transform duration-500 group-hover:scale-[1.02]" sizes="(max-width:1024px) 50vw, 25vw" />) : (<div className="absolute inset-0 bg-gradient-to-br from-gray-700 to-gray-800"><div className="absolute inset-0 flex items-center justify-center"><div className="text-center"><div className="w-16 h-16 mx-auto mb-3 bg-gray-600 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg></div><p className="text-neutral-500 text-xs font-medium">Fantasy Football</p></div></div></div>)}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/10 via-transparent to-transparent pointer-events-none" />
                      <div className="absolute top-3 right-3"><svg className="w-4 h-4 text-neutral-600 group-hover:text-neutral-900 transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></div>
                    </div>
                    <div className="px-1">
                      <h3 className="text-base 2xl:text-lg 3xl:text-xl font-bold text-neutral-900 line-clamp-2 group-hover:text-neutral-700 transition-colors duration-300 mb-2">{article.title}</h3>
                      {article.summary && !hideSummaries && (<p className="text-xs 2xl:text-sm 3xl:text-base line-clamp-2 text-neutral-500 leading-relaxed">{article.summary}</p>)}
                      {article.author?.name && (<p className="text-xs text-gray-500 mt-2 font-medium">By {article.author.name}</p>)}
                    </div>
                  </div>
                </Link>
              ) : (
                <div className="space-y-3"><div className="h-[200px] 2xl:h-[220px] 3xl:h-[250px] rounded-xl bg-neutral-100 flex items-center justify-center"><p className="text-neutral-500">No content available</p></div><div className="px-1"><p className="text-gray-500 text-sm">Content unavailable</p></div></div>
              )}
            </div>
          ))}
          {fantasyArticles && fantasyArticles.length < 4 && Array.from({ length: 4 - fantasyArticles.length }).map((_, index) => (
            <div key={`placeholder-${index}`} className="space-y-3"><div className="h-[200px] 2xl:h-[220px] 3xl:h-[250px] rounded-xl bg-neutral-100 flex items-center justify-center"><div className="text-center"><div className="w-16 h-16 mx-auto mb-3 bg-neutral-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg></div></div></div><div className="px-1"><p className="text-gray-500 text-sm">Fantasy content coming soon</p></div></div>
          ))}
        </div>
      </div>
    </section>
