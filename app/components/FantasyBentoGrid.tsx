...
export default async function FantasyBentoGrid({ textureSrc, hideSummaries = false }: FantasyBentoGridProps) {
                ...
                <Link href={`/fantasy/${article.slug.current}`} className="group">
                  <div className="space-y-3">
                    <div className="relative h-[200px] 2xl:h-[220px] 3xl:h-[250px] rounded-xl overflow-hidden bg-gray-900 hover:bg-gray-800 transition-all duration-500 hover:scale-[1.01] shadow-xl hover:shadow-2xl">
                      {article.coverImage?.asset ? (
                        <Image
                          ...
                          alt={article.title}
                          fill
                          className="object-cover object-left-top opacity-90 group-hover:opacity-100 transition-all duration-500"
                    ...
                    <div className="px-1">
                      ...
                      {article.summary && !hideSummaries && (
                        <p className="text-xs 2xl:text-sm 3xl:text-base line-clamp-2 text-gray-400 leading-relaxed">{article.summary}</p>
                      )}
                      {article.author?.name && (
                        <p className="text-xs text-gray-500 mt-2 font-medium">By {article.author.name}</p>
                      )}
                    </div>
                  </div>
                </Link>
