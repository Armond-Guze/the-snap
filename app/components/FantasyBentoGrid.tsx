...
export default async function FantasyBentoGrid({ textureSrc }: FantasyBentoGridProps) {
  ...
  const fantasyQuery = `*[_type == "headline" && published == true] | order(_createdAt desc)[0...5]{
    _id,
    title,
    slug,
    summary,
  ...
  const leftArticles = fantasyArticles?.slice(0, 2) || []; // Two for left side
  const centerArticle = fantasyArticles?.[2]; // One for center
  const rightArticles = fantasyArticles?.slice(3, 5) || []; // Two for right side
